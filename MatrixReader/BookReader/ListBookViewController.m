...
- (void)bookBtnIsPressed:(id)sender
{
    ...
    NSMutableDictionary *book = [self.books objectAtIndex:[(UIButton *)sender tag]];
    NSLog(@"%@",book);
    NSString *typeBook = [book objectForKey:@"typeBook"];
    NSString *nameBook = [[NSBundle mainBundle] pathForResource:[@"BookDatas/MoYan/" stringByAppendingString:[book objectForKey:@"fileBook"]] ofType:typeBook];
    ...
    */
    ...
    else if ([typeBook isEqualToString:@"txt"]){
        DetailBookTxtViewController *detailBookWebView = [[DetailBookTxtViewController alloc] initWithTxtName:nameBook];
        detailBookWebView.title=[book objectForKey:@"nameBook"];
        [self.navigationController pushViewController:detailBookWebView animated:YES];
        [detailBookWebView release];
    }
...
//    }
    ...
    if (self.typeTable == DefaultStoreMode) {
        url = [NSString stringWithFormat:URL_GET_LIST_BOOKS_OF_CATEGORY, self.idCategory];
    } else if (self.typeTable == MyStoreMode) {
        url = [NSString stringWithFormat:URL_GET_LIST_DOWNLOADED_BOOK, userIDFB];
    }
