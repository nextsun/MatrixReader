...
- (void)btnCategoryIsPressed:(id)sender
{
    ...
    */
    ...
    appDelegate.arrBooks =  appDelegate.arrBooks = [[[NSMutableArray alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"BookDatas/BookList" ofType:@"plist" ]] autorelease];;
...
- (void)btnRightIsPressed:(id)sender
{
    ...
    switch ([(UIButton *)sender tag]) {
        ...
        case 1001:
            ...
            appDelegate.isDownloadOwnBooks = YES;
            //[appDelegate.arrBooks release];
            appDelegate.arrBooks = [[[NSMutableArray alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"BookDatas/BookList" ofType:@"plist" ]] autorelease];
            
                
                //[listBookView setIdCategory:[(UIButton *)sender tag]];
                [listBookView setTypeTable:MyStoreMode];
                [listBookView setArrOwnEbooks:appDelegate.arrBooks];
             NSLog(@"%@",self.arrOwnBooks);
                UINavigationController *listBookNavigationView = [[UINavigationController alloc] initWithRootViewController:listBookView];
                [listBookView release];
...
- (void)viewDidLoad
{
    ...
    //[self performSelector:@selector(btnCategoryIsPressed:)];
